...
centroids = detect_craters(pre, 10, 500);

%% Create Images
%% show original
imshow(image);

%% show preprossing mask
imshow(pre);
...
caption = "Site 4 Full Image: Untuned Red Mask";
...
caption = "Site 4 Full Image: Untuned Centroids";
title(caption, "FontSize", 14);
hold off
...
function f = preprocess(image, color_channel, bins, blur_std, trim_threshold)
    ...
    step3 = step2 < trim_threshold; %trim noise
    f = step3;
end
